...
@pytest.mark.skip('test_guess_file_encoding')
def test_guess_file_encoding() -> None:
    """ test guess_file_encoding """
    _logger.info(test_guess_file_encoding.__name__)
    study_configurations: list[dict[str, any]] = json.loads(_config.get('STUDY_CONFIGURATIONS', '[]'))
    study_configurations = [sc for sc in study_configurations if sc.get('active', True)]
    file_encoding: str = C3dcEtl.guess_file_encoding(
        study_configurations[0].get('transformations', [])[0].get('source_file_path')
    )
    assert file_encoding == 'iso-8859-1'

@pytest.mark.skip('test_get_petl_table_from_source_file')
def test_get_petl_table_from_source_file() -> None:
    """ test get_petl_table_from_source_file """
    _logger.info(test_get_petl_table_from_source_file.__name__)
    study_configurations: list[dict[str, any]] = json.loads(_config.get('STUDY_CONFIGURATIONS', '[]'))
    study_configurations = [sc for sc in study_configurations if sc.get('active', True)]
    tbl: any = C3dcEtl.get_petl_table_from_source_file(
        study_configurations[0].get('transformations', [])[0].get('source_file_path')
    )
    assert tbl
